...
<form action="{{Route('Store Curriculum')}}" method="post">
    ...
    <table class="vertical-table mx-auto mt-5">
        <tr>
            <td><button class="">Insert Curriculum</button></td>
        </tr>
        <tr>
            <td class="border-0 pb-3 font-weight-bold" colspan="7">Insert Curriculum</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="details border border-dark" colspan="6">
                <input id="prerequisite" class="w-100" type="text" name="course[prerequisite]" value="@if(isset($course) && !$course->prerequisite->isEmpty()) {{$course->prerequisite}} @endif"/>
            </td>
        </tr>
        <tr>
            ...
            @if(!isset($course))
                @for($i = 1; $i <= 3; $i++)
                    <td class="ch-label border border-dark">
                        <select id="contact_type_select{{$i}}" class="w-100 contactType" name="course[contactType][]">
                            <option selected></option>
                            <option>Lecture</option>
                            <option>TEL</option>
                            <option>Tutorial</option>
                            <option>Lab</option>
                            <option>Example Class</option>
                        </select>
                    </td>
    ...
    <table class="vertical-table mx-auto mt-5">
        ...
        <tr>
            <td class="border border-dark">
                <textarea id="course_aims" name="course[aims]" class="w-100" type="text">@if(isset($course) && $course->objectives !== null){{$course->objectives->courseAims}}@endif</textarea>
            </td>
        </tr>
        <tr>
                ...
                <table id="contents" class="mt-3 w-100">
                    ...
                    @if(isset($course) && !$course->content->isEmpty())
                        @foreach($course->content as $key=>$content)
                            <tr id="topic{{$content->ID}}" class="content_topic">
                                ...
                                <td class="border border-dark">
                                    <textarea class="contentTextArea" name="content[topic][]" placeholder="Topic description">{{$content->topics}}</textarea>
                                </td>
                                <td class="border border-dark">
                                    <textarea class="contentTextArea" name="content[details1][]" placeholder="Topic details">@if(isset($content->contentAttDetails->details1)){{$content->contentAttDetails->details1}}@endif</textarea>
                                </td>
                                <td class="border border-dark att2" @if($content->contentAttDetails->rowspan == 1) rowspan="1" @elseif($content->contentAttDetails->rowspan == 2) rowspan="2" @elseif($content->contentAttDetails->rowspan == 0) style="display:none" @endif>
                                    <textarea class="contentTextArea" name="content[details2][]" placeholder="Topic details">@if(isset($content->contentAttDetails->details2)){{$content->contentAttDetails->details2}}@endif</textarea>
                    ...
                    @else
                        <tr id="topic1" class="content_topic">
                            ...
                            <td class="border border-dark">
                                <textarea class="contentTextArea" name="content[topic][]" placeholder="Topic description"></textarea>
                            </td>
                            <td class="border border-dark">
                                <textarea class="contentTextArea" name="content[details1][]" placeholder="Topic details"></textarea>
                            </td>
                            <td class="border border-dark att2">
                                <textarea class="contentTextArea" name="content[details2][]" placeholder="Topic details"></textarea>
                ...
                <table id="assessment" class="mt-3 w-100">
                            ...
                            <tr id="assessment{{$assessment->ID}}" class="assessment_row">
                                ...
                                <td class="border border-dark text-center LO_col">
                                    @foreach($course->learningOutcomes as $LO_key=>$LO)
                                        <label><input id="a{{$assessment->ID}}LO{{($LO_key+1)}}" type="checkbox" name="assessment[{{$assessment->ID}}LO][]" value="{{$LO->ID}}" @if(in_array($LO->ID, $assessment->lo_tested)) checked @endif/>&emsp;{{$LO->ID}}<br></label>
                                    @endforeach
                                </td>
                                ...
                                <td class="border border-dark weightCol">
                                    <input type="text" id="w{{$assessment->ID}}" class="assessmentWeight" name="assessment[weight][]" placeholder="Percentage" value="{{$assessment->weightage}}"/>
                                </td>
                                ...
                                <td class="border border-dark rubricsCol">
                                    <textarea name="assessment[rubrics][]" placeholder="Rubrics info to be appended at the end of the document">@if(isset($assessment->rubrics->description)){{$assessment->rubrics->description}}@endif</textarea>
                                </td>
                                <td class="border border-dark">
                                    @if($key > 0)
                    ...
                    @else
                        <tr id="assessment1" class="assessment_row">
                            ...
                            <td class="border border-dark weightCol">
                                <input type="text" id="w1" class="assessmentWeight" name="assessment[weight][]" placeholder="Percentage"/>
                            </td>
                            <td class="border border-dark pl-3">
                                <input id="assessment1Individual" type="radio" name="assessment[1category]" value="individual" checked/>
                                <label for="assessment1Individual">Individual</label><br>
                                <input id="assessment1Team" type="radio" name="assessment[1category]" value="team"/>
                                <label for="assessment1Team">Team</label>
                            </td>
                            <td class="border border-dark rubricsCol">
                                <textarea name="assessment[rubrics][]" placeholder="Rubrics info to be appended at the end of the document"></textarea>
                            </td>
                            <td class="border border-dark">
                                <input id="addNewAssessment" type="button" value="Add"/>
                            </td>
            ...
            <td id="mappingGradAttr" class="border border-dark border-top-0 p-1">
                <table class="w-100">
                    ...
                    <tr>
                        ...
                        @endforeach
                        <td class='short-col border border-dark text-center'>@if(isset($course) and in_array('CE', $course->codeSplit)) <img src='{{ asset('assets/full-dot.png') }}' style='width:17px' alt=""> @endif</td>
                        <td class='short-col border border-dark text-center'>@if(isset($course) and in_array('CZ', $course->codeSplit)) <img src='{{ asset('assets/full-dot.png') }}' style='width:17px' alt=""> @endif</td>
                    </tr>
                    <!-- Forth Row -->
                    <tr id="">
                        <td class='long-col border border-dark font-weight-bold'>Overall Statement</td>
                        <td id="mapping_overall_statement" class='border border-dark' colspan="{{$grad_attr->count()+1}}">@if(isset($course) and $course->objectives->courseAims !== null){!! nl2br(e($course->objectives->courseAims)) !!}@endif</td>
                    ...
                    @if(isset($course) and !$course->learningOutcomes->isEmpty())
                        @foreach($course->learningOutcomes as $key => $learningOutcome)
                            <tr id="rowMapLO{{$learningOutcome->ID}}" class="LO_gradAttr">
                                ...
                                <td id="mapLOGradAttr" class="border border-dark" colspan="{{($grad_attr->count())+1}}">
                                    <span class="text-primary">Hover mouse over text for more info.</span><br/>
                                    @foreach($grad_attr as $element)
                                        <label class="cbGradAttrLbl">
    ...
    <table class="vertical-table mx-auto mt-5">
        ...
        <tr>
            <td id="formativeFeedback" class="border border-dark">
                <textarea name="formativeFeedback" placeholder="Formative Feedback">@if(isset($course) and isset($course->formativeFeedback)){!! nl2br(e($course->formativeFeedback->description)) !!}@endif</textarea>
            </td>
        </tr>
    </table>
                ...
                <table id="instructor" class="border border-dark mx-auto w-100">
                    ...
                    @if(isset($course) and !$course->instructor->isempty())
                        @foreach($course->instructor as $key=>$instructor)
                            <tr id="instructor{{$instructor->ID}}" class="instructor_row">
                                ...
                                <td>
                                    @if($key > 0)
                                        <input id="instructor{{$instructor->ID}}" class="btn-danger instructor_remove" type="button" value="Del"/>
                                    @else
                ...
                <table id="schedule" class="border border-dark mx-auto w-100">
                    ...
                    @if(isset($course) and !$course->schedule->isempty())
                        @foreach($course->schedule as $key=>$schedule)
                            <tr id="schedule{{$schedule->ID}}" class="schedule_row">
                                ...
                                <td class="border border-dark LO_col text-center">
                                    <!-- LO -->
                                    @foreach($course->learningOutcomes as $LO_key=>$LO)
                                        <label><input id="s{{$schedule->ID}}LO{{($LO_key+1)}}" type="checkbox" name="schedule[{{$schedule->ID}}LO][]" value="{{$LO->ID}}" @if(in_array($LO->ID, $schedule->LO)) checked @endif/>&emsp;{{$LO->ID}}<br></label>
                                    @endforeach
                                </td>
                                <td class="border border-dark">
                                ...
                                <td class="border border-dark action">
                                    @if($key > 0)
                                        <input id="schedule{{$schedule->ID}}" class="btn-danger schedule_remove" type="button" value="Del"/>
                                    @else
                        ...
                        @foreach($course->appendix as $key=>$appendix)
                            ...
                            <tr id="appendix{{$appendix->ID}}Description" class="appendix_description_row">
                                ...
                                <td class="border border-dark border-top-0 action">
                                    <input id="appendix{{$appendix->ID}}" class="addNewCriteria" type="button" value="Add"/>Criteria Table
                                </td>
                            </tr>
                                    ...
                                    <table class="w-100 border border-dark">
                                        ...
                                        @foreach($appendix->criteria as $ckey=>$criteria)
                                            <tr id="appendix{{$appendix->ID}}Criteria{{$criteria->ID}}" class="appendix'+appendix_id+'_criteria_row">
                                                <td class="w-25 border border-dark mid-col"><textarea name="appendix[{{$appendix->ID}}criteria][]" placeholder="Assessment">{{$criteria->header}}</textarea></td>
                                                <td class="w-25 border border-dark mid-col"><textarea name="appendix[{{$appendix->ID}}fail][]" placeholder="Fail Standards">{{$criteria->fail}}</textarea></td>
                                                <td class="w-25 border border-dark mid-col"><textarea name="appendix[{{$appendix->ID}}pass][]" placeholder="Pass Standards">{{$criteria->pass}}</textarea></td>
                                                <td class="w-25 border border-dark mid-col"><textarea name="appendix[{{$appendix->ID}}high][]" placeholder="High Standards">{{$criteria->high}}</textarea></td>
                                                <td class="border border-dark action">
                                                @if($ckey > 0)
                                                    <input id="appendix{{$appendix->ID}}Criteria{{$criteria->ID}}" class="btn-danger criteriaRow_remove" type="button" value="Del" />
                                                @else
                                                    <input id="appendix'+appendix_id+'" class="addNewCriteriaRow" type="button" value="Add" />
                                                @endif
                                                Criteria Row</td>
                                            </tr>
                                        @endforeach
